...
void DroneSimpleController::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  if (!_sdf->HasElement("bodyName"))
  {
    ...
    link = boost::dynamic_pointer_cast<physics::Link>(world->BaseByName(link_name_));
  }
...
void DroneSimpleController::ImuCallback(const sensor_msgs::ImuConstPtr& imu)
{
  ...
  angular_velocity = pose.Rot().RotateVector(ignition::math::Vector3<double>(imu->angular_velocity.x, imu->angular_velocity.y, imu->angular_velocity.z));
}
...
void DroneSimpleController::UpdateDynamics(double dt){
    ignition::math::Vector3<double> force, torque;
  ...
  //  if (imu_subscriber_.getTopic()=="")
   ...
   // if (state_topic_.empty())
    ...
    ignition::math::Vector3<double> body_vel = pose.Rot().RotateVector(velocity);
    ignition::math::Vector3<double> body_acc = pose.Rot().RotateVector(acceleration);
    ...
    ignition::math::Vector3<double> poschange = pose.Pos() - position;
    position = pose.Pos();
    
  
    // Get gravity
    world->SetPhysicsEnabled(true);
   ignition::math::Vector3<double> gravity_body = pose.Rot().RotateVector(world->Gravity());
   ...
   ignition::math::Quaternion<double> heading_quaternion(cos(euler.Z()/2),0,0,sin(euler.Z()/2));
   ignition::math::Vector3<double> velocity_xy = heading_quaternion.RotateVectorReverse(velocity);
   ignition::math::Vector3<double> acceleration_xy = heading_quaternion.RotateVectorReverse(acceleration);
   ignition::math::Vector3<double> angular_velocity_body = pose.Rot().RotateVectorReverse(angular_velocity);
    ...
    if( m_posCtrl){
        ...
        if(navi_state == FLYING_MODEL){
            ...
            ignition::math::Vector3<double> vb = heading_quaternion.RotateVectorReverse(ignition::math::Vector3<double>(vx,vy,vz));
...
void DroneSimpleController::Reset()
{
  ...
  link->SetForce(ignition::math::Vector3<double>(0,0,0));
  link->SetTorque(ignition::math::Vector3<double>(0,0,0));
